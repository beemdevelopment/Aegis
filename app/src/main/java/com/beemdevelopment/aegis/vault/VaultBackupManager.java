...
public class VaultBackupManager {
    ...
    public void scheduleBackup(File tempFile) {
        _executor.execute(() -> {
            try {
    ...
    private void createBackup(File tempFile)
            throws VaultRepositoryException, VaultBackupPermissionException {
        if (_prefs.isSingleBackupEnabled()) {
            Uri fileUri = _prefs.getBackupFileLocation();
            if (fileUri == null) {
                throw new VaultRepositoryException("getBackupFileLocation returned null");
            }
            createBackup(tempFile, fileUri);
        } else {
            Uri dirUri = _prefs.getBackupsLocation();
            if (dirUri == null) {
